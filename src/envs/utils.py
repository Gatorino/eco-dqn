...
class G1GraphGenerator(GraphGenerator):
    ...
    def get(self, with_padding=False):
        matrix_invariance = self.permu
        if matrix_invariance:
            ...
            np.random.seed(1)
            print("Not performing permutation on graph, seed 1")
...
class RandomGraphGenerator(GraphGenerator):
    ...
    def get(self, with_padding=False):
        ...
        if self.biased:
            bias = np.array([self.get_w() if np.random.uniform()
                            < density else 0 for _ in range(self.n_spins)])
...
class RandomErdosRenyiGraphGenerator(GraphGenerator):
    ...
    def __init__(self, n_spins=20, p_connection=[0.1, 0], edge_type=EdgeType.DISCRETE):
        ...
        if self.edge_type == EdgeType.UNIFORM:
            self.get_connection_mask = lambda: np.ones(
                (self.n_spins, self.n_spins))
        elif self.edge_type == EdgeType.DISCRETE:
            def get_connection_mask():
                mask = 2. * \
                    np.random.randint(
                        2, size=(self.n_spins, self.n_spins)) - 1.
...
class SetGraphGenerator(GraphGenerator):
    ...
    def __init__(self, matrices, biases=None, ordered=False):
        ...
        if len(set([m.shape[0]-1 for m in matrices])) == 1:
            ...
            raise NotImplementedError(
                "All graphs in SetGraphGenerator must have the same dimension.")
        ...
        if not self.biased:
            ...
            assert len(matrices) == len(
                biases), "Must pass through the same number of matrices and biases."
            assert all([len(b) == self.n_spins+1 for b in biases]
                       ), "All biases and must have the same dimension as the matrices."
...
class PerturbedGraphGenerator(GraphGenerator):
    ...
    def __init__(self, matrices, perturb_mean=0, perturb_std=0.01, biases=None, ordered=False):
        ...
        if len(set([m.shape[0] - 1 for m in matrices])) == 1:
            ...
            raise NotImplementedError(
                "All graphs passed to PerturbedGraphGenerator must have the same dimension.")
        ...
        if not self.biased:
            ...
            raise NotImplementedError(
                "Not implemented PerturbedGraphGenerator for biased graphs yet.")
    ...
    def get(self, with_padding=False):
        ...
        noise = np.random.normal(
            self.perturb_mean, self.perturb_std, size=m.shape)
